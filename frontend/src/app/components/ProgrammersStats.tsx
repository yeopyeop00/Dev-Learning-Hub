...
export function ProgrammersStats() {
        ...
        <div className="flex flex-col items-center justify-center h-[400px] border border-border rounded-lg bg-card/50 gap-4">
          ...
          <div className="text-center">
            ...
            <p className="text-sm text-muted-foreground mt-1">
              우측 상단의 <span className="inline-flex items-center gap-1 font-medium text-foreground"><Settings className="w-3 h-3" />깃허브 설정</span> 버튼을 눌러 레포지토리를 연결하세요
            </p>
          </div>
        </div>
      </div>
  ...
  const monthlyData = [
    { id: "jan", month: "1월", solved: 32 },
    { id: "feb", month: "2월", solved: 41 },
    { id: "mar", month: "3월", solved: 28 },
  ];

  const levelDistribution = [
    { level: "Lv.0", count: 45, color: "bg-gray-400" },
    { level: "Lv.1", count: 68, color: "bg-green-400" },
    { level: "Lv.2", count: 52, color: "bg-blue-400" },
    { level: "Lv.3", count: 28, color: "bg-purple-500" },
    { level: "Lv.4", count: 12, color: "bg-orange-500" },
    { level: "Lv.5", count: 3, color: "bg-red-500" },
  ];
  ...
  const grassGrid = Array.from({ length: 52 }, (_, week) =>
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-muted-foreground">
          {githubConfig ? `${githubConfig.programmersRepo} · 문제 풀이 기록 및 레벨 분포` : "문제 풀이 기록 및 레벨 분포"}
        </p>
      </div>
        ...
        <div className="bg-card rounded-lg border border-border p-6">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-2xl font-bold">Lv.2</p>
            </div>
          </div>
        </div>
        <div className="bg-card rounded-lg border border-border p-6">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-2xl font-bold">28문제</p>
            </div>
          </div>
        </div>
      </div>
            ...
            <div key={level.level}>
              ...
              <div className="w-full bg-muted rounded-full h-3">
                <div
                  ...
                  style={{ width: `${(level.count / 208) * 100}%` }}
                ></div>
              </div>
            </div>
          ))}
              ...
              <div className="flex items-start justify-between">
                <div className="flex-1">
                  ...
                  <div className="flex items-center gap-3 mt-2 text-sm text-muted-foreground">
                    <span className={`px-2 py-1 rounded ${getLevelColor(problem.level)}`}>
                      {problem.level}
                    </span>
