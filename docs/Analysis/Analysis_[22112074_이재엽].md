...
| 06/04/2026 | 1.1 | 클래스 및 EXTENSION SCENARIOS 추가 | |
...
| Step | Branching Action |
|------|------------------|
| 2 | 2a. DB에 저장된 데이터가 없을 경우<br>2a1. 시스템이 데이터가 없는 빈 대시보드 화면을 출력한다. |
...
| Step | Branching Action |
|------|------------------|
| 2 | 2a. 저장된 할 일 항목이 없을 경우<br>2a1. 시스템이 등록된 할 일이 없다는 안내 메시지를 화면에 출력한다. |
...
| Step | Branching Action |
|------|------------------|
| 1 | 1a. 유저가 할 일 내용을 입력하지 않고 추가 버튼을 클릭할 경우<br>1a1. 내용이 입력되지 않으면 추가 버튼이 비활성화되어 기능이 실행되지 않는다. |
...
| Step | Branching Action |
|------|------------------|
| 2 | 2a. DB 처리 중 오류가 발생할 경우<br>2a1. 시스템이 삭제에 실패했다는 오류 메시지를 화면에 출력한다.<br>2a2. 이전과 동일한 to-do list 화면을 유지한다. |
...
| Step | Branching Action |
|------|------------------|
| 2 | 2a. 저장된 일정이 없을 경우<br>2a1. 시스템이 등록된 일정이 없는 빈 달력 화면을 출력한다. |
...
| Success Post Condition | 성공적으로 일정이 추가된 달력 화면이 출력된다. |
| Failed Post Condition | 이전과 같은 달력 화면이 출력된다. |
...
| Primary Actor | User |
| Preconditions | 로그인 된 상태여야 한다. |
| Trigger | 캘린더 화면에서 각 일정의 삭제 버튼을 클릭시 |
| Success Post Condition | 삭제가 성공적으로 반영된 달력 화면이 출력된다. |
| Failed Post Condition | 이전과 같은 달력 화면이 출력된다. |
...
| Step | Branching Action |
|------|------------------|
| 2 | 2a. DB 처리 중 오류가 발생할 경우<br>2a1. 시스템이 삭제에 실패했다는 오류 메시지를 화면에 출력한다.<br>2a2. 이전과 동일한 달력 화면을 유지한다. |
...
| Step | Branching Action |
|------|------------------|
| 2 | 2a. 저장된 시간표 데이터가 없을 경우<br>2a1. 시스템이 등록된 수업이 없는 빈 시간표 화면을 출력한다. |
...
| Step | Branching Action |
|------|------------------|
| 2 | 2a. DB 처리 중 오류가 발생할 경우<br>2a1. 시스템이 삭제에 실패했다는 오류 메시지를 화면에 출력한다.<br>2a2. 이전과 동일한 시간표 화면을 유지한다. |
...
| 3 | 3a. 동기화 완료 후 변경된 데이터가 없을 경우<br>3a1. 시스템이 변경된 데이터가 없다는 안내 메시지를 팝업으로 출력한다. |
...
19) **ParsedCommit**: 커밋 메시지 하나를 파싱한 결과를 담는 클래스다. ...: 커밋 메시지 하나를 파싱한 결과를 담는 클래스다. 문제 제목, 난이도(레벨), 풀이 시각 정보를 보관하며, CommitMessageParser가 생성하고 GithubSyncController가 이를 집계해 ProgrammersStat을 갱신하는 데 활용한다.
